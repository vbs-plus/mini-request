...
/**
 * @param errMsg 微信接口回调消息
 * @param statusCode 微信接口回调状态
 * @param status 支付宝接口回调状态
 */
interface RequestBaseSuccessRes {
  errMsg: string;
  statusCode?: number;
  status?: number;
}
 ...
 * @template TTask 操作的任务类型
 */
export class CacheOperator<
  TRes extends RequestBaseSuccessRes = RequestBaseSuccessRes,
  TOptions extends RequestOptions = RequestOptions, // 操作函数
  TTask extends RequestTask = RequestTask // 操作的任务类型
> {
  /**
   * 缓存配置
   */
  public readonly config: ICacheConfiguration<TRes, TOptions>;

  private readonly op: (option: TOptions) => TTask;
  private readonly cache: Cacher<TRes & CacheRes> = new Cacher();
  /**
   * 正在处理的回调
   */
  private readonly callbackListMap: {
    [key: string]: {
      success: Function[];
      fail: Function[];
      complete: Function[];
   ...
   */
  constructor(
    operator: (option: TOptions) => TTask,
    config?: ICacheConfiguration<TRes, TOptions>
  ) {
    this.op = operator;
    this.config = config || {
      expire: 15 * 60 * 1000,
  ...
  public static createHandler<
    TRes extends RequestBaseSuccessRes = RequestBaseSuccessRes,
    TOptions extends RequestOptions = RequestOptions, // 微信操作函数
    TTask extends RequestTask = RequestTask // 微信操作的任务类型
  >(
    operator: (option: TOptions) => TTask,
    config?: ICacheConfiguration<TRes, TOptions>
  ): CacheOperator<TRes, TOptions, TTask>["handle"] {
   ...
   */
  public handle(
    options: TOptions & Partial<ICacheConfiguration<TRes, TOptions>>
  ): TTask {
    const keyBuilder =
      options.keyBuilder || this.config.keyBuilder || defaultKeyBuilder;
    ...
    if (result) {
      ...
      const data = {
        ...
        success: (res: TRes) => {
          const expire =
            options.expire === undefined ? this.config.expire : options.expire;
          // 过期时间为0不缓存,但是会合并请求
          if (
            expire > 0 &&
            (options.resultCondition || this.config.resultCondition)(res)
          ) {
            // 缓存请求结果
            this.cache.set(key, res, expire);
          }
          this._getMapBeforeComplete(key).success.forEach((v) => {
            v(res);
          });
        },
        fail: (res: { errMsg: string }) => {
          // fail 回调异步化 (兼容部分版本同步调用)
          // tslint:disable-next-line: no-floating-promises
          Promise.resolve(this._getMapBeforeComplete(key).fail).then((f) => {
            f.forEach((v) => {
              v(res);
            });
          });
        ...
        complete: (res: TRes) => {
          this.completeMap[key].forEach((v) => {
            v(res);
          });
   ...
   * @param key cacheKey
   */
  private _getMapBeforeComplete(key: string): {
    success: Function[];
    fail: Function[];
    complete: Function[];
  } {
 ...
 * @returns string
 */
export function defaultKeyBuilder<
...
>(opts: TOptions): string {
  ...
  const data =
    (opts as { data?: any }).data || (opts as { formData?: any }).formData;
  return (
    JSON.stringify(opts, CACHE_FIELDS) + (data ? JSON.stringify(data) : "")
  );
}
 ...
 */
// TODO
export function isOkResult(res: RequestBaseSuccessRes): boolean {
