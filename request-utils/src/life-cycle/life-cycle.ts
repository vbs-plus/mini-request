...
export abstract class LifeCycle<
  ...
  TInitConfig extends ILCBaseConfiguration<TFullOptions, TWxOptions>,
  TFullOptions extends TInitConfig & ILCExtraConfiguration = TInitConfig &
    ILCExtraConfiguration //完整配置项
> {
  /**
   * 默认全局配置
  ...
  private _onSend(
    ...
    return Promise.resolve(options).then(options.transformSend);
  }
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      data.fail = (res: wx.HttpResponse): void => {
        ...
        if (cancelToken && cancelToken.isCancelled()) {
          ...
          // 还有重试次数
          this._send<T>(data, options).then(resolve, reject);
          return;
        }
        // 结束请求
        completed = true;
  ...
  private _onComplete(
    ...
    this.Listeners.onComplete.forEach((f) => {
      f(res as any, options);
    });
  }
  ...
  private _onAbort(reason: any, options: TFullOptions): void {
    // tslint:disable-next-line: no-unsafe-any
    this.Listeners.onAbort.forEach((f) => {
      f(reason, options);
    });
  }
}
